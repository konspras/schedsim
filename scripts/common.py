...
class SimParams:
    ...
    # quantums_to_sweep = [0.1, 100000.0]
    quantums_to_sweep = [0.1, 1.0, 10.0, 100.0, 1000.0, 10000.0, 100000.0]
    ...
    def get_experiment_dirname(self):
        self.validate()
        d = os.path.join(self.output_dir, f"topo{self.topo}_ll{self.load_level}_mu{self.mu}_gen{self.gen_type}_prc{self.proc_type}_c{self.cores}_ctx{self.ctx_cost}_wl{self.cdfWorkload}_dur{self.duration}_{self.sweep_type.name}")
